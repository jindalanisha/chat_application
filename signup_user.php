<?php
	...
	$con=mysqli_connect("localhost","root","root","chatapp") or die("connection not established");
		...
		$check_email= "select * from users where user_email='$email'"; //checks if this email is present in the database
		$run_email=mysqli_query($con,$check_email); //run this query
		...
		if($query){
			...
			echo "<script>window.open('signin.php','_self')</script>"; //redirect to sign in
		}else{
			echo "<script>alert('Registration failed')</script>";
			echo "<script>window.open('signup.php','_self')</script>"; //redirect
		}
